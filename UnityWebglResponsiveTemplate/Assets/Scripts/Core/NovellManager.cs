...
namespace UVNF.Core
{
    public class NovellManager : MonoBehaviour
    {
        [Header("UDSF Components")]
        public NovellCanvas Canvas;
        ...
        [SerializeField] private GameObject city;[SerializeField] private GameObject bg;
        ...
        private void OnGUI()
        {
            if (GUI.Button(new Rect(Screen.width/2, 25, 60, 60), lifeIconTexture))
            {
                _resetStory();
                bg.SetActive(false);
            }
        }
        public void _resetStory()
        {
            FinishStory();
            if (StartingStory != null)
                StartStory(StartingStory);
    ...
    {
        private NovellManager _manager;
        private NovellCanvas _canvas;
        ...
        public UVNFStoryManager(StoryGraph graph, NovellManager manager, NovellCanvas canvas, Action afterStoryHandler)
        {
            _storyGraph = graph;
        ...
        public void CreateSubStory(StoryGraph graph, NovellManager manager, NovellCanvas canvas)
        {
