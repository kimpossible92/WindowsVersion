...
public class UiSpriteFrameAnimator : MonoBehaviour
{
    ...
    [SerializeField]
    private UIDocument uiDocument1;
    [SerializeField] private UIDocument uiDocument2 = null;
    [SerializeField] private UnityEngine.UI.Image bg_Image;
    [SerializeField]
    private UnityEngine.UI.Image bg_Image2;
    ...
    {
        elemButton = new VisualElement[15];
        #region Label2
        bg_Image.gameObject.SetActive(false);
        bg_Image2.gameObject.SetActive(false);
        uiDocument2.GetComponent<uiDoc2>().NewEnable();
        elemButton[0] = GetComponent<UIDocument>().rootVisualElement.Query<VisualElement>("button-1");
        if (elemButton[0] != null)
        {
            elemButton[0].RegisterCallback<PointerUpEvent>(OnPointerUp1);
        }
        elemButton[1] = GetComponent<UIDocument>().rootVisualElement.Query<VisualElement>("button-2");
        if (elemButton[1] != null)
        {
            elemButton[1].RegisterCallback<PointerUpEvent>(OnPointerUp2);
        }

        #endregion
        uiDocument1.enabled = false;
    ...
    private void OnPointerUp1(PointerUpEvent evt)
    {
        UnityEngine.SceneManagement.SceneManager.LoadScene("sample");
    }
    private void OnPointerUp2(PointerUpEvent evt)
    {
        uiDocument2.enabled = true;
        uiDocument1.enabled = true;
        uiDocument2.GetComponent<uiDoc2>().EnableUI();
        uiDocument.enabled = false;

        elemButton[2] = uiDocument1.GetComponent<UIDocument>().rootVisualElement.Query<VisualElement>("0");
        elemButton[3] = uiDocument1.GetComponent<UIDocument>().rootVisualElement.Query<VisualElement>("1");
        if (elemButton[2] != null)
        {
            elemButton[2].RegisterCallback<PointerUpEvent>(delegate
            {
                uiDocument1.GetComponent<UnityEngine.UI.Button>().onClick.Invoke();
            });
