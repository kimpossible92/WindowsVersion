...
public class WheelAlignment : MonoBehaviour {
    ...
    void alignMeshToCollider()
    {
        ...
        if (Physics.Raycast(wheelCol.transform.position, -wheelCol.transform.up, out hit, SuspensionDistance + wheelCol.radius))
        {
            wheelGraphic.transform.position = hit.point + wheelCol.transform.up *  wheelCol.radius;
        }
        else
        {
            wheelGraphic.transform.position = wheelCol.transform.position - (wheelCol.transform.up * SuspensionDistance);
        }
